...
void AskTurn(bool& _TURN,string& NamePlayer_O,string& NamePlayer_X) { // Hỏi lượt đánh trước
	...
	DrawAsciiFile(x + wide / 2, y + high / 2 - 2, "DrawTurnX", BLUE);
	// Vẽ ô hiển thị tên
	int WideBoxName = 30;
	int HighBoxName = 3;
	int X_BoxName = 44;
		...
		if (c == 'A' || c == 'D') {
			if (c == 'A') {
				if (x == 50) {
					...
					/*GotoXY(x + wide / 2, y + high / 2);*/
					DrawAsciiFile(x + wide / 2, y + high / 2 - 2, "DrawTurnO", RED);
				}
			}
			else if (c == 'D') {
				if (x == 20) {
...
void TurnX() {
	/*VeO1(GRAY);
	VeX2(RED);*/
	DrawAsciiFile(0, 17, "VeO1", BLUE);
	DrawAsciiFile(0, 17, "VeX2", RED);
}
void TurnO() {
	/*VeO2(BLUE);
	VeX1(GRAY);*/
	DrawAsciiFile(0, 17, "VeO2", RED);
	DrawAsciiFile(0, 17, "VeX1", BLUE);
}
